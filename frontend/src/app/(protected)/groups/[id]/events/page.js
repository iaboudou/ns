...
export default function GroupEvents({ params }) {
  ...
  return (
    <>
      <button className={styles.secondaryButton} onClick={() => (showCreateEvent ? setShowCreateEvent(false) : setShowCreateEvent(true))}>
        Create Event
      </button>
      ...
      {events.length === 0 ? (
        ...
        <>
          {events.map((e) => (
            <EventCard key={e.id} event={e} groupId={id} />
          ))}
        </>
      )}

      {hasMore && (
        <button className={styles.loadMoreBtn} onClick={() => handleFetchData(events, setEvents, setLoading, setHasMore, 'events', id)} disabled={loading}>
